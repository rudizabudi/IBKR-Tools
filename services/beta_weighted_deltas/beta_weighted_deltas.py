...
def calculate_beta(core: Core, positions: list[Position], con: TWSCon):
    ...
    request_historical_data(core=core, con=con, position=benchmark_positon)
    ...
    for pos in beta_pos_dummies.keys():
        if pos not in core.pos_betas.keys():
            request_historical_data(core=core, con=con, position=beta_pos_dummies[pos])
...
def get_portfolio_positions(core: Core, con: TWSCon):
    core.raw_positions = {}
...
def request_historical_data(core: Core, con: TWSCon, position: Position):
